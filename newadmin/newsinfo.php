...
class cnews extends cTable {
	...
	function __construct() {
		...
		// thumbnail
		$this->_thumbnail = new cField('news', 'news', 'x__thumbnail', 'thumbnail', '`thumbnail`', '`thumbnail`', 200, -1, FALSE, '`thumbnail`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// status
		$this->status = new cField('news', 'news', 'x_status', 'status', '`status`', '`status`', 16, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["news_id"]))
				$arKeys[] = ew_StripSlashes($_POST["news_id"]);
			elseif (isset($_GET["news_id"]))
				$arKeys[] = ew_StripSlashes($_GET["news_id"]);
			else
