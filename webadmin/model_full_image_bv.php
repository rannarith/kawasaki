...
$model_full_image_blobview = NULL; // Initialize page object first

class cmodel_full_image_blobview extends cmodel {
	...
	// Page object name
	var $PageObjName = 'model_full_image_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('model', 'x_full_image');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('full_image');
				...
				if (trim(strval($this->Recordset->fields('full_image'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('full_image') . "\"");
				}
...
// Create page object
if (!isset($model_full_image_blobview)) $model_full_image_blobview = new cmodel_full_image_blobview();

// Page init
$model_full_image_blobview->Page_Init();

// Page main
$model_full_image_blobview->Page_Main();
?>
<?php
$model_full_image_blobview->Page_Terminate();
?>
