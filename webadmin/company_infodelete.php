	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$sThisKey .= $row['cid'];
				$this->logo->UploadPath = '../assets/images/';
				@unlink(ew_UploadPathEx(TRUE, $this->logo->UploadPath) . $row['logo']);
				$conn->raiseErrorFn = 'ew_ErrorFn';
...
// Page object
var company_info_delete = new ew_Page("company_info_delete");
company_info_delete.PageID = "delete"; // Page ID
var EW_PAGE_ID = company_info_delete.PageID; // For backward compatibility

// Form object
var fcompany_infodelete = new ew_Form("fcompany_infodelete");
...
fcompany_infodelete.ValidateRequired = true;
<?php } else { ?>
fcompany_infodelete.ValidateRequired = false; 
<?php } ?>
...
if ($company_info_delete->Recordset = $company_info_delete->LoadRecordset())
	$company_info_deleteTotalRecs = $company_info_delete->Recordset->RecordCount(); // Get record count
if ($company_info_deleteTotalRecs <= 0) { // No record found, exit
	if ($company_info_delete->Recordset)
		$company_info_delete->Recordset->Close();
	$company_info_delete->Page_Terminate("company_infolist.php"); // Return to list
}
?>
<p><span class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Delete") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $company_info->TableCaption() ?></span></p>
...
?>
<form name="fcompany_infodelete" id="fcompany_infodelete" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<p>
...
<table cellspacing="0" id="tbl_company_infodelete" class="ewTable ewTableSeparate">
...
?>
	<tr<?php echo $company_info->RowAttributes() ?>>
		<td<?php echo $company_info->cid->CellAttributes() ?>><span id="el<?php echo $company_info_delete->RowCnt ?>_company_info_cid" class="company_info_cid">
<span<?php echo $company_info->cid->ViewAttributes() ?>>
<?php echo $company_info->cid->ListViewValue() ?></span>
</span></td>
		<td<?php echo $company_info->company_name->CellAttributes() ?>><span id="el<?php echo $company_info_delete->RowCnt ?>_company_info_company_name" class="company_info_company_name">
<span<?php echo $company_info->company_name->ViewAttributes() ?>>
<?php echo $company_info->company_name->ListViewValue() ?></span>
</span></td>
		<td<?php echo $company_info->logo->CellAttributes() ?>><span id="el<?php echo $company_info_delete->RowCnt ?>_company_info_logo" class="company_info_logo">
<span>
...
<?php } ?>
		...
		<td<?php echo $company_info->_email->CellAttributes() ?>><span id="el<?php echo $company_info_delete->RowCnt ?>_company_info__email" class="company_info__email">
<span<?php echo $company_info->_email->ViewAttributes() ?>>
<?php echo $company_info->_email->ListViewValue() ?></span>
</span></td>
		<td<?php echo $company_info->phone_number->CellAttributes() ?>><span id="el<?php echo $company_info_delete->RowCnt ?>_company_info_phone_number" class="company_info_phone_number">
<span<?php echo $company_info->phone_number->ViewAttributes() ?>>
<?php echo $company_info->phone_number->ListViewValue() ?></span>
</span></td>
		<td<?php echo $company_info->status->CellAttributes() ?>><span id="el<?php echo $company_info_delete->RowCnt ?>_company_info_status" class="company_info_status">
