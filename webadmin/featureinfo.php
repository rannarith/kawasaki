...
class cfeature extends cTable {
	...
	function __construct() {
		...
		// feature_id
		$this->feature_id = new cField('feature', 'feature', 'x_feature_id', 'feature_id', '`feature_id`', '`feature_id`', 3, -1, FALSE, '`feature_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->feature_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['feature_id'] = &$this->feature_id;

		// top_title
		$this->top_title = new cField('feature', 'feature', 'x_top_title', 'top_title', '`top_title`', '`top_title`', 200, -1, FALSE, '`top_title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['top_title'] = &$this->top_title;

		// title
		$this->title = new cField('feature', 'feature', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['title'] = &$this->title;

		// model_id
		$this->model_id = new cField('feature', 'feature', 'x_model_id', 'model_id', '`model_id`', '`model_id`', 3, -1, FALSE, '`model_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->model_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['model_id'] = &$this->model_id;

		// thumbnail
		$this->thumbnail = new cField('feature', 'feature', 'x_thumbnail', 'thumbnail', '`thumbnail`', '`thumbnail`', 200, -1, TRUE, '`thumbnail`', FALSE, FALSE, FALSE, 'IMAGE');
		$this->fields['thumbnail'] = &$this->thumbnail;

		// long_des
		$this->long_des = new cField('feature', 'feature', 'x_long_des', 'long_des', '`long_des`', '`long_des`', 201, -1, FALSE, '`long_des`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['long_des'] = &$this->long_des;

		// f_order
		$this->f_order = new cField('feature', 'feature', 'x_f_order', 'f_order', '`f_order`', '`f_order`', 16, -1, FALSE, '`f_order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			$sql .= ew_QuotedName('feature_id') . '=' . ew_QuotedValue($rs['feature_id'], $this->feature_id->FldDataType) . ' AND ';
		}
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@feature_id@", ew_AdjustSql($this->feature_id->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl("featureview.php", $this->UrlParm());
	}

	// Add URL
	function GetAddUrl() {
		return "featureadd.php";
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		return $this->KeyUrl("featureedit.php", $this->UrlParm($parm));
	}
	...
	function GetCopyUrl($parm = "") {
		return $this->KeyUrl("featureadd.php", $this->UrlParm($parm));
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = @$_GET["feature_id"]; // feature_id
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				$Doc->ExportCaption($this->feature_id);
				$Doc->ExportCaption($this->top_title);
				...
				$Doc->ExportCaption($this->f_order);
			} else {
				$Doc->ExportCaption($this->feature_id);
				$Doc->ExportCaption($this->top_title);
				$Doc->ExportCaption($this->title);
