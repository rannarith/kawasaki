...
class cmodel_add extends cmodel {
	...
	function ValidateForm() {
		...
		if ($this->getCurrentDetailTable() == "specification" && $GLOBALS["specification"]->DetailAdd) {
		...
		if ($this->getCurrentDetailTable() == "feature" && $GLOBALS["feature"]->DetailAdd) {
		...
		if ($this->getCurrentDetailTable() == "model_gallery" && $GLOBALS["model_gallery"]->DetailAdd) {
		...
		if ($this->getCurrentDetailTable() == "model_color" && $GLOBALS["model_color"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($this->model_logo->Upload->Action == "1") { // Keep
			if ($rsold) {
				$rsnew['model_logo'] = $rsold->fields['model_logo'];
			}
		...
		if ($this->icon_menu->Upload->Action == "1") { // Keep
			if ($rsold) {
				$rsnew['icon_menu'] = $rsold->fields['icon_menu'];
			}
		...
		if ($this->full_image->Upload->Action == "1") { // Keep
			if ($rsold) {
				$rsnew['full_image'] = $rsold->fields['full_image'];
			}
		...
		if ($AddRow) {
			...
			$rsnew['model_id'] = $this->model_id->DbValue;
		}
...
// Form object
var fmodeladd = new ew_Form("fmodeladd");

// Validate form
fmodeladd.Validate = function(fobj) {
...
fmodeladd.ValidateRequired = true;
<?php } else { ?>
fmodeladd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fmodeladd.Lists["x_category_id"] = {"LinkField":"x_category_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_category_name","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<p><span class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $model->TableCaption() ?></span></p>
...
?>
<form name="fmodeladd" id="fmodeladd" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return ewForms[this.id].Submit();">
<p>
<input type="hidden" name="t" value="model">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table cellspacing="0" id="tbl_modeladd" class="ewTable">
...
<script type="text/javascript">
ew_CreateEditor("fmodeladd", "x_description", 50, 6, <?php echo ($model->description->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
...
<script type="text/javascript">
fmodeladd.Lists["x_category_id"].Options = <?php echo (is_array($model->category_id->EditValue)) ? ew_ArrayToJson($model->category_id->EditValue, 1) : "[]" ?>;
</script>
...
<p>
<?php if ($model->getCurrentDetailTable() == "specification" && $specification->DetailAdd) { ?>
<br>
<?php include_once "specificationgrid.php" ?>
<br>
<?php } ?>
<?php if ($model->getCurrentDetailTable() == "feature" && $feature->DetailAdd) { ?>
<br>
<?php include_once "featuregrid.php" ?>
<br>
<?php } ?>
<?php if ($model->getCurrentDetailTable() == "model_gallery" && $model_gallery->DetailAdd) { ?>
<br>
<?php include_once "model_gallerygrid.php" ?>
<br>
<?php } ?>
<?php if ($model->getCurrentDetailTable() == "model_color" && $model_color->DetailAdd) { ?>
<br>
