...
$model_icon_menu_blobview = NULL; // Initialize page object first

class cmodel_icon_menu_blobview extends cmodel {
	...
	// Page object name
	var $PageObjName = 'model_icon_menu_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('model', 'x_icon_menu');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('icon_menu');
				...
				if (trim(strval($this->Recordset->fields('icon_menu'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('icon_menu') . "\"");
				}
...
// Create page object
if (!isset($model_icon_menu_blobview)) $model_icon_menu_blobview = new cmodel_icon_menu_blobview();

// Page init
$model_icon_menu_blobview->Page_Init();

// Page main
$model_icon_menu_blobview->Page_Main();
?>
<?php
$model_icon_menu_blobview->Page_Terminate();
?>
