...
$model_model_logo_blobview = NULL; // Initialize page object first

class cmodel_model_logo_blobview extends cmodel {
	...
	// Page object name
	var $PageObjName = 'model_model_logo_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('model', 'x_model_logo');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('model_logo');
				...
				if (trim(strval($this->Recordset->fields('model_logo'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('model_logo') . "\"");
				}
...
// Create page object
if (!isset($model_model_logo_blobview)) $model_model_logo_blobview = new cmodel_model_logo_blobview();

// Page init
$model_model_logo_blobview->Page_Init();

// Page main
$model_model_logo_blobview->Page_Main();
?>
<?php
$model_model_logo_blobview->Page_Terminate();
?>
