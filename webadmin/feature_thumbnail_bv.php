...
$feature_thumbnail_blobview = NULL; // Initialize page object first

class cfeature_thumbnail_blobview extends cfeature {
	...
	// Page object name
	var $PageObjName = 'feature_thumbnail_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('feature', 'x_thumbnail');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (!EW_DEBUG_ENABLED && ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('thumbnail');
				...
				if (trim(strval($this->Recordset->fields('thumbnail'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('thumbnail') . "\"");
				}
...
// Create page object
if (!isset($feature_thumbnail_blobview)) $feature_thumbnail_blobview = new cfeature_thumbnail_blobview();

// Page init
$feature_thumbnail_blobview->Page_Init();

// Page main
$feature_thumbnail_blobview->Page_Main();
?>
<?php
$feature_thumbnail_blobview->Page_Terminate();
?>
