...
class cnews extends cTable {
	...
	function __construct() {
		...
		// news_id
		$this->news_id = new cField('news', 'news', 'x_news_id', 'news_id', '`news_id`', '`news_id`', 3, -1, FALSE, '`news_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->news_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['news_id'] = &$this->news_id;

		// thumbnail
		$this->thumbnail = new cField('news', 'news', 'x_thumbnail', 'thumbnail', '`thumbnail`', '`thumbnail`', 200, -1, TRUE, '`thumbnail`', FALSE, FALSE, FALSE, 'IMAGE');
		...
		// short_des
		$this->short_des = new cField('news', 'news', 'x_short_des', 'short_des', '`short_des`', '`short_des`', 201, -1, FALSE, '`short_des`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['short_des'] = &$this->short_des;

		// long_des
		$this->long_des = new cField('news', 'news', 'x_long_des', 'long_des', '`long_des`', '`long_des`', 201, -1, FALSE, '`long_des`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['long_des'] = &$this->long_des;

		// youtube_url
		$this->youtube_url = new cField('news', 'news', 'x_youtube_url', 'youtube_url', '`youtube_url`', '`youtube_url`', 200, -1, FALSE, '`youtube_url`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->fields['youtube_url'] = &$this->youtube_url;

		// news_date
		$this->news_date = new cField('news', 'news', 'x_news_date', 'news_date', '`news_date`', '`news_date`', 200, -1, FALSE, '`news_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
		$this->news_date->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateDMY"));
		$this->fields['news_date'] = &$this->news_date;

		// status
		$this->status = new cField('news', 'news', 'x_status', 'status', '`status`', '`status`', 16, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT');
	...
	function GetDetailUrl() {
		...
		if ($this->getCurrentDetailTable() == "news_gallery") {
			$sDetailUrl = $GLOBALS["news_gallery"]->GetListUrl() . "?showmaster=" . $this->TableVar;
			$sDetailUrl .= "&news_id=" . $this->news_id->CurrentValue;
		}
		return $sDetailUrl;
	}
	...
	function DeleteSQL(&$rs, $where = "") {
		...
		if ($rs) {
			$sql .= ew_QuotedName('news_id') . '=' . ew_QuotedValue($rs['news_id'], $this->news_id->FldDataType) . ' AND ';
		}
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@news_id@", ew_AdjustSql($this->news_id->CurrentValue), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	// View URL
	function GetViewUrl() {
		return $this->KeyUrl("newsview.php", $this->UrlParm());
	}
	...
	function GetEditUrl($parm = "") {
		if ($parm <> "")
			...
			return $this->KeyUrl("newsedit.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function GetCopyUrl($parm = "") {
		if ($parm <> "")
			...
			return $this->KeyUrl("newsadd.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	}
	...
	function ExportDocument(&$Doc, &$Recordset, $StartRec, $StopRec, $ExportPageType = "") {
		...
		if ($Doc->Horizontal) { // Horizontal format, write header
			...
			if ($ExportPageType == "view") {
				...
				$Doc->ExportCaption($this->news_date);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if ($ExportPageType == "view") {
					...
					$Doc->ExportField($this->news_date);
