...
function CurrentProjectID() {
	...
	return "{5CB4AA5B-9E5D-4D28-80A0-11AF7C90D6A1}";
}
...
// Table header
	...
	function ExportField(&$fld) {
		...
		if ($this->Horizontal) {
			...
			$this->Text .= "<td" . ((EW_EXPORT_CSS_STYLES) ? $fld->CellStyles() : "") . ">" . $fld->ExportValue() . "</td></tr>";
		}
	}
...
class cEmail {
	...
	function Send() {
		...
		$result = ew_SendEmail($this->Sender, $this->Recipient, $this->Cc, $this->Bcc,
			...
			$this->AttachmentFileName, $this->AttachmentContent);
...
class cTableBase {
	...
	var $BasicSearchKeywordDefault = ""; // Basic search keyword default
	var $BasicSearchTypeDefault = "="; // Basic search keyword default
...
class cField {
	...
	function getAdvancedSearch($name) {
		$fldparm = $this->UrlParameterName($name);
		return @$_SESSION[EW_PROJECT_NAME . "_" . $this->TblVar . "_" . EW_TABLE_ADVANCED_SEARCH . "_" . $fldparm];
	}

	function setAdvancedSearch($name, $v) {
		$fldparm = $this->UrlParameterName($name);
		if (@$_SESSION[EW_PROJECT_NAME . "_" . $this->TblVar . "_" . EW_TABLE_ADVANCED_SEARCH . "_" . $fldparm] <> $v)
			$_SESSION[EW_PROJECT_NAME . "_" . $this->TblVar . "_" . EW_TABLE_ADVANCED_SEARCH . "_" . $fldparm] = $v;
	}
...
class cAdvancedSecurity {
	...
	function ValidateUser($usr, $pwd, $autologin) {
		...
		if (!$ValidateUser) {
			...
			$sSql = $admin_user->GetSQL($sFilter, "");
...
function ew_SendEmail($sFrEmail, $sToEmail, $sCcEmail, $sBccEmail, $sSubject, $sMail, $sFormat, $sCharset, $sSmtpSecure = "", $sAttachmentFileName = "", $sAttachmentContent = "") {
