...
class cnews_gallery extends cTable {
	...
	function __construct() {
		...
		// ng_id
		$this->ng_id = new cField('news_gallery', 'news_gallery', 'x_ng_id', 'ng_id', '`ng_id`', '`ng_id`', 3, -1, FALSE, '`ng_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->ng_id->Sortable = TRUE; // Allow sort
		$this->ng_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['ng_id'] = &$this->ng_id;

		// news_id
		$this->news_id = new cField('news_gallery', 'news_gallery', 'x_news_id', 'news_id', '`news_id`', '`news_id`', 3, -1, FALSE, '`news_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// image
		$this->image = new cField('news_gallery', 'news_gallery', 'x_image', 'image', '`image`', '`image`', 200, -1, TRUE, '`image`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->image->Sortable = TRUE; // Allow sort
		$this->fields['image'] = &$this->image;

		// g_order
		$this->g_order = new cField('news_gallery', 'news_gallery', 'x_g_order', 'g_order', '`g_order`', '`g_order`', 16, -1, FALSE, '`g_order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->g_order->Sortable = TRUE; // Allow sort
		$this->g_order->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['g_order'] = &$this->g_order;

		// g_status
		$this->g_status = new cField('news_gallery', 'news_gallery', 'x_g_status', 'g_status', '`g_status`', '`g_status`', 16, -1, FALSE, '`g_status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->g_status->Sortable = TRUE; // Allow sort
		$this->g_status->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->g_status->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->g_status->OptionCount = 2;
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`news_gallery`";
	}
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`ng_id` DESC";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('ng_id', $rs))
				ew_AddFilter($where, ew_QuotedName('ng_id', $this->DBID) . '=' . ew_QuotedValue($rs['ng_id'], $this->ng_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (is_null($this->ng_id->CurrentValue))
			...
			$sKeyFilter = str_replace("@ng_id@", ew_AdjustSql($this->ng_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "news_galleryedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "news_galleryadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("news_galleryview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("news_galleryview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "news_galleryadd.php?" . $this->UrlParm($parm);
		else
			$url = "news_galleryadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("news_galleryedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("news_galleryadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function AddMasterUrl($url) {
		if ($this->getCurrentMasterTable() == "news" && strpos($url, EW_TABLE_SHOW_MASTER . "=") === FALSE) {
	...
	function KeyToJson() {
		$json = "";
		$json .= "ng_id:" . ew_VarToJson($this->ng_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["ng_id"]))
				$arKeys[] = $_POST["ng_id"];
			elseif (isset($_GET["ng_id"]))
				$arKeys[] = $_GET["ng_id"];
			else
	...
	function RenderEditRow() {
		...
		$this->g_order->EditValue = $this->g_order->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->ng_id->Exportable) $Doc->ExportCaption($this->ng_id);
					if ($this->news_id->Exportable) $Doc->ExportCaption($this->news_id);
					if ($this->image->Exportable) $Doc->ExportCaption($this->image);
					if ($this->g_order->Exportable) $Doc->ExportCaption($this->g_order);
					if ($this->g_status->Exportable) $Doc->ExportCaption($this->g_status);
				} else {
					if ($this->ng_id->Exportable) $Doc->ExportCaption($this->ng_id);
					if ($this->news_id->Exportable) $Doc->ExportCaption($this->news_id);
					if ($this->image->Exportable) $Doc->ExportCaption($this->image);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->ng_id->Exportable) $Doc->ExportField($this->ng_id);
						if ($this->news_id->Exportable) $Doc->ExportField($this->news_id);
						if ($this->image->Exportable) $Doc->ExportField($this->image);
						if ($this->g_order->Exportable) $Doc->ExportField($this->g_order);
						if ($this->g_status->Exportable) $Doc->ExportField($this->g_status);
					} else {
						if ($this->ng_id->Exportable) $Doc->ExportField($this->ng_id);
						if ($this->news_id->Exportable) $Doc->ExportField($this->news_id);
						if ($this->image->Exportable) $Doc->ExportField($this->image);
