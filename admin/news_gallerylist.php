...
class cnews_gallery_list extends cnews_gallery {
	...
	var $FormName = 'fnews_gallerylist';
	...
	function __construct() {
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fnews_gallerylistsrch";
	...
	function GetFilterList() {
		...
		$sFilterList = "";
		$sSavedFilterList = "";
		$sFilterList = ew_Concat($sFilterList, $this->ng_id->AdvancedSearch->ToJson(), ","); // Field ng_id
		...
		$sFilterList = ew_Concat($sFilterList, $this->g_order->AdvancedSearch->ToJson(), ","); // Field g_order
		$sFilterList = ew_Concat($sFilterList, $this->g_status->AdvancedSearch->ToJson(), ","); // Field g_status
		if ($this->BasicSearch->Keyword <> "") {
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fnews_gallerylistsrch", $filters);
	...
	function RestoreFilterList() {
		...
		// Field ng_id
		$this->ng_id->AdvancedSearch->SearchValue = @$filter["x_ng_id"];
		$this->ng_id->AdvancedSearch->SearchOperator = @$filter["z_ng_id"];
		$this->ng_id->AdvancedSearch->SearchCondition = @$filter["v_ng_id"];
		$this->ng_id->AdvancedSearch->SearchValue2 = @$filter["y_ng_id"];
		$this->ng_id->AdvancedSearch->SearchOperator2 = @$filter["w_ng_id"];
		$this->ng_id->AdvancedSearch->Save();
		...
		// Field g_order
		$this->g_order->AdvancedSearch->SearchValue = @$filter["x_g_order"];
		$this->g_order->AdvancedSearch->SearchOperator = @$filter["z_g_order"];
		$this->g_order->AdvancedSearch->SearchCondition = @$filter["v_g_order"];
		$this->g_order->AdvancedSearch->SearchValue2 = @$filter["y_g_order"];
		$this->g_order->AdvancedSearch->SearchOperator2 = @$filter["w_g_order"];
		$this->g_order->AdvancedSearch->Save();

		// Field g_status
		$this->g_status->AdvancedSearch->SearchValue = @$filter["x_g_status"];
		$this->g_status->AdvancedSearch->SearchOperator = @$filter["z_g_status"];
		$this->g_status->AdvancedSearch->SearchCondition = @$filter["v_g_status"];
		$this->g_status->AdvancedSearch->SearchValue2 = @$filter["y_g_status"];
		$this->g_status->AdvancedSearch->SearchOperator2 = @$filter["w_g_status"];
		$this->g_status->AdvancedSearch->Save();
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" class=\"ewMultiSelect\" value=\"" . ew_HtmlEncode($this->ng_id->CurrentValue) . "\" onclick=\"ew_ClickMultiCheckbox(event);\">";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fnews_gallerylistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fnews_gallerylistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fnews_gallerylist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fnews_gallerylistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function RenderRow() {
		...
		$this->g_status->ViewCustomAttributes = "";
			...
			if ($this->image->UseColorbox) {
				...
				$this->image->LinkAttrs["data-rel"] = "news_gallery_x" . $this->RowCnt . "_image";
...
var CurrentForm = fnews_gallerylist = new ew_Form("fnews_gallerylist", "list");
fnews_gallerylist.FormKeyCountName = '<?php echo $news_gallery_list->FormKeyCountName ?>';
...
fnews_gallerylist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fnews_gallerylist.Lists["x_news_id"] = {"LinkField":"x_news_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_title","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"news"};
fnews_gallerylist.Lists["x_news_id"].Data = "<?php echo $news_gallery_list->news_id->LookupFilterQuery(FALSE, "list") ?>";
fnews_gallerylist.Lists["x_g_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fnews_gallerylist.Lists["x_g_status"].Options = <?php echo json_encode($news_gallery_list->g_status->Options()) ?>;

// Form object for search
var CurrentSearchForm = fnews_gallerylistsrch = new ew_Form("fnews_gallerylistsrch");
</script>
<script type="text/javascript">
...
<div class="ewToolbar">
<?php if ($news_gallery_list->TotalRecs > 0 && $news_gallery_list->ExportOptions->Visible()) { ?>
<?php $news_gallery_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($news_gallery_list->SearchOptions->Visible()) { ?>
<?php $news_gallery_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($news_gallery_list->FilterOptions->Visible()) { ?>
<?php $news_gallery_list->FilterOptions->Render("body") ?>
<?php } ?>
...
<?php } ?>
<?php
	$bSelectLimit = $news_gallery_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($news_gallery_list->TotalRecs <= 0)
			$news_gallery_list->TotalRecs = $news_gallery->ListRecordCount();
	} else {
		if (!$news_gallery_list->Recordset && ($news_gallery_list->Recordset = $news_gallery_list->LoadRecordset()))
	...
	if ($news_gallery->CurrentAction == "" && $news_gallery_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$news_gallery_list->setWarningMessage(ew_DeniedMsg());
		if ($news_gallery_list->SearchWhere == "0=101")
			$news_gallery_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$news_gallery_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$news_gallery_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($news_gallery->Export == "" && $news_gallery->CurrentAction == "") { ?>
<form name="fnews_gallerylistsrch" id="fnews_gallerylistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($news_gallery_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fnews_gallerylistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="news_gallery">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($news_gallery_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($news_gallery_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $news_gallery_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($news_gallery_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($news_gallery_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($news_gallery_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($news_gallery_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
?>
<?php if ($news_gallery_list->TotalRecs > 0 || $news_gallery->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($news_gallery_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> news_gallery">
<div class="box-header ewGridUpperPanel">
<?php if ($news_gallery->CurrentAction <> "gridadd" && $news_gallery->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($news_gallery_list->Pager)) $news_gallery_list->Pager = new cPrevNextPager($news_gallery_list->StartRec, $news_gallery_list->DisplayRecs, $news_gallery_list->TotalRecs, $news_gallery_list->AutoHidePager) ?>
<?php if ($news_gallery_list->Pager->RecordCount > 0 && $news_gallery_list->Pager->Visible) { ?>
<div class="ewPager">
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $news_gallery_list->PageUrl() ?>start=<?php echo $news_gallery_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($news_gallery_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $news_gallery_list->PageUrl() ?>start=<?php echo $news_gallery_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $news_gallery_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($news_gallery_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $news_gallery_list->PageUrl() ?>start=<?php echo $news_gallery_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($news_gallery_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $news_gallery_list->PageUrl() ?>start=<?php echo $news_gallery_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $news_gallery_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($news_gallery_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $news_gallery_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $news_gallery_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $news_gallery_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($news_gallery_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
</div>
<form name="fnews_gallerylist" id="fnews_gallerylist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($news_gallery_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $news_gallery_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="news_gallery">
<?php if ($news_gallery->getCurrentMasterTable() == "news" && $news_gallery->CurrentAction <> "") { ?>
...
<?php if ($news_gallery_list->TotalRecs > 0 || $news_gallery->CurrentAction == "gridedit") { ?>
<table id="tbl_news_gallerylist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$news_gallery_list->RowType = EW_ROWTYPE_HEADER;
...
<?php if ($news_gallery->ng_id->Visible) { // ng_id ?>
	...
	<?php } else { ?>
		<th data-name="ng_id" class="<?php echo $news_gallery->ng_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $news_gallery->SortUrl($news_gallery->ng_id) ?>',1);"><div id="elh_news_gallery_ng_id" class="news_gallery_ng_id">
...
<?php if ($news_gallery->news_id->Visible) { // news_id ?>
	...
	<?php } else { ?>
		<th data-name="news_id" class="<?php echo $news_gallery->news_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $news_gallery->SortUrl($news_gallery->news_id) ?>',1);"><div id="elh_news_gallery_news_id" class="news_gallery_news_id">
...
<?php if ($news_gallery->image->Visible) { // image ?>
	...
	<?php } else { ?>
		<th data-name="image" class="<?php echo $news_gallery->image->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $news_gallery->SortUrl($news_gallery->image) ?>',1);"><div id="elh_news_gallery_image" class="news_gallery_image">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $news_gallery->image->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($news_gallery->image->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($news_gallery->image->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($news_gallery->g_order->Visible) { // g_order ?>
	...
	<?php } else { ?>
		<th data-name="g_order" class="<?php echo $news_gallery->g_order->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $news_gallery->SortUrl($news_gallery->g_order) ?>',1);"><div id="elh_news_gallery_g_order" class="news_gallery_g_order">
...
<?php if ($news_gallery->g_status->Visible) { // g_status ?>
	...
	<?php } else { ?>
		<th data-name="g_status" class="<?php echo $news_gallery->g_status->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $news_gallery->SortUrl($news_gallery->g_status) ?>',1);"><div id="elh_news_gallery_g_status" class="news_gallery_g_status">
...
<span id="el<?php echo $news_gallery_list->RowCnt ?>_news_gallery_ng_id" class="news_gallery_ng_id">
...
<span id="el<?php echo $news_gallery_list->RowCnt ?>_news_gallery_news_id" class="news_gallery_news_id">
...
<span id="el<?php echo $news_gallery_list->RowCnt ?>_news_gallery_image" class="news_gallery_image">
<span>
...
<span id="el<?php echo $news_gallery_list->RowCnt ?>_news_gallery_g_order" class="news_gallery_g_order">
...
<span id="el<?php echo $news_gallery_list->RowCnt ?>_news_gallery_g_status" class="news_gallery_g_status">
...
<?php } ?>
<?php if ($news_gallery_list->TotalRecs == 0 && $news_gallery->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($news_gallery_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
fnews_gallerylistsrch.FilterList = <?php echo $news_gallery_list->GetFilterList() ?>;
