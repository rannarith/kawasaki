...
class cmodel_color extends cTable {
	...
	function __construct() {
		...
		// mc_id
		$this->mc_id = new cField('model_color', 'model_color', 'x_mc_id', 'mc_id', '`mc_id`', '`mc_id`', 3, -1, FALSE, '`mc_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->mc_id->Sortable = TRUE; // Allow sort
		$this->mc_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['mc_id'] = &$this->mc_id;

		// image
		$this->image = new cField('model_color', 'model_color', 'x_image', 'image', '`image`', '`image`', 200, -1, TRUE, '`image`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->image->Sortable = TRUE; // Allow sort
		$this->fields['image'] = &$this->image;

		// model_id
		$this->model_id = new cField('model_color', 'model_color', 'x_model_id', 'model_id', '`model_id`', '`model_id`', 3, -1, FALSE, '`model_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// color_code
		$this->color_code = new cField('model_color', 'model_color', 'x_color_code', 'color_code', '`color_code`', '`color_code`', 200, -1, FALSE, '`color_code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->color_code->Sortable = TRUE; // Allow sort
		$this->fields['color_code'] = &$this->color_code;

		// m_order
		$this->m_order = new cField('model_color', 'model_color', 'x_m_order', 'm_order', '`m_order`', '`m_order`', 16, -1, FALSE, '`m_order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// m_status
		$this->m_status = new cField('model_color', 'model_color', 'x_m_status', 'm_status', '`m_status`', '`m_status`', 16, -1, FALSE, '`m_status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->m_status->Sortable = TRUE; // Allow sort
		$this->m_status->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->m_status->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->m_status->OptionCount = 2;
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`model_color`";
	}
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`mc_id` DESC,`model_id` ASC";
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('mc_id', $rs))
				ew_AddFilter($where, ew_QuotedName('mc_id', $this->DBID) . '=' . ew_QuotedValue($rs['mc_id'], $this->mc_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (is_null($this->mc_id->CurrentValue))
			...
			$sKeyFilter = str_replace("@mc_id@", ew_AdjustSql($this->mc_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "model_coloredit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "model_coloradd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("model_colorview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("model_colorview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "model_coloradd.php?" . $this->UrlParm($parm);
		else
			$url = "model_coloradd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("model_coloredit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("model_coloradd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "mc_id:" . ew_VarToJson($this->mc_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["mc_id"]))
				$arKeys[] = $_POST["mc_id"];
			elseif (isset($_GET["mc_id"]))
				$arKeys[] = $_GET["mc_id"];
			else
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->mc_id->Exportable) $Doc->ExportCaption($this->mc_id);
					if ($this->image->Exportable) $Doc->ExportCaption($this->image);
					if ($this->model_id->Exportable) $Doc->ExportCaption($this->model_id);
					if ($this->color_code->Exportable) $Doc->ExportCaption($this->color_code);
					if ($this->m_order->Exportable) $Doc->ExportCaption($this->m_order);
					if ($this->m_status->Exportable) $Doc->ExportCaption($this->m_status);
				} else {
					if ($this->mc_id->Exportable) $Doc->ExportCaption($this->mc_id);
					if ($this->image->Exportable) $Doc->ExportCaption($this->image);
					if ($this->model_id->Exportable) $Doc->ExportCaption($this->model_id);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->mc_id->Exportable) $Doc->ExportField($this->mc_id);
						if ($this->image->Exportable) $Doc->ExportField($this->image);
						if ($this->model_id->Exportable) $Doc->ExportField($this->model_id);
						if ($this->color_code->Exportable) $Doc->ExportField($this->color_code);
						if ($this->m_order->Exportable) $Doc->ExportField($this->m_order);
						if ($this->m_status->Exportable) $Doc->ExportField($this->m_status);
					} else {
						if ($this->mc_id->Exportable) $Doc->ExportField($this->mc_id);
						if ($this->image->Exportable) $Doc->ExportField($this->image);
						if ($this->model_id->Exportable) $Doc->ExportField($this->model_id);
