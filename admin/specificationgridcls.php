...
$specification_grid = NULL; // Initialize page object first

class cspecification_grid extends cspecification {
	...
	// Page object name
	var $PageObjName = 'specification_grid';

	// Grid form hidden field names
	var $FormName = 'fspecificationgrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["specification"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->spec_id->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_title") && $objForm->HasValue("o_title") && $this->title->CurrentValue <> $this->title->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_s_order") && $objForm->HasValue("o_s_order") && $this->s_order->CurrentValue <> $this->s_order->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->spec_id->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->spec_id->OldValue = $this->spec_id->CurrentValue;
		...
		$this->s_order->OldValue = $this->s_order->CurrentValue;
	...
	function LoadFormValues() {
		...
		if (!$this->spec_id->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->s_order->setOldValue($objForm->GetValue("o_s_order"));
	...
	function NewRow() {
		...
		$row = array();
		$row['spec_id'] = $this->spec_id->CurrentValue;
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->spec_id->CurrentValue = strval($arKeys[0]); // spec_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
