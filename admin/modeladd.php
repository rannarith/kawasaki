...
$model_add = NULL; // Initialize page object first

class cmodel_add extends cmodel {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["model_id"] != "") {
				...
				$this->setKey("model_id", $this->model_id->CurrentValue); // Set up key
			} else {
				$this->setKey("model_id", ""); // Clear key
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "modelview.php")
	...
	function LoadDefaultValues() {
		...
		$this->model_logo->CurrentValue = NULL; // Clear file related field
		...
		$this->icon_menu->CurrentValue = NULL; // Clear file related field
		...
		$this->full_image->CurrentValue = NULL; // Clear file related field
	...
	function RenderRow() {
		...
		$this->is_available->ViewCustomAttributes = "";
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->model_logo);
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->icon_menu);
			...
			if (($this->CurrentAction == "I" || $this->CurrentAction == "C") && !$this->EventCancelled) ew_RenderUploadField($this->full_image);
	...
	function ValidateForm() {
		...
		if (in_array("model_color", $DetailTblVar) && $GLOBALS["model_color"]->DetailAdd) {
		...
		if (in_array("model_gallery", $DetailTblVar) && $GLOBALS["model_gallery"]->DetailAdd) {
		...
		if (in_array("feature", $DetailTblVar) && $GLOBALS["feature"]->DetailAdd) {
		...
		if (in_array("specification", $DetailTblVar) && $GLOBALS["specification"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// displacement
		$this->displacement->SetDbValueDef($rsnew, $this->displacement->CurrentValue, NULL, FALSE);
		...
		if ($this->model_logo->Visible && !$this->model_logo->Upload->KeepFile) {
			$this->model_logo->Upload->DbValue = ""; // No need to delete old file
		...
		if ($this->icon_menu->Visible && !$this->icon_menu->Upload->KeepFile) {
			$this->icon_menu->Upload->DbValue = ""; // No need to delete old file
		...
		if ($this->full_image->Visible && !$this->full_image->Upload->KeepFile) {
			$this->full_image->Upload->DbValue = ""; // No need to delete old file
		...
		// seo_description
		$this->seo_description->SetDbValueDef($rsnew, $this->seo_description->CurrentValue, NULL, FALSE);

		// seo_keyword
		$this->seo_keyword->SetDbValueDef($rsnew, $this->seo_keyword->CurrentValue, NULL, FALSE);
		...
		if ($this->model_logo->Visible && !$this->model_logo->Upload->KeepFile) {
			...
			if (!ew_Empty($this->model_logo->Upload->FileName)) {
				...
				$this->model_logo->SetDbValueDef($rsnew, $this->model_logo->Upload->FileName, NULL, FALSE);
			}
		}
		if ($this->icon_menu->Visible && !$this->icon_menu->Upload->KeepFile) {
			$this->icon_menu->UploadPath = '../assets/images/model_menu/';
			...
			if (!ew_Empty($this->icon_menu->Upload->FileName)) {
				...
				$this->icon_menu->SetDbValueDef($rsnew, $this->icon_menu->Upload->FileName, NULL, FALSE);
			}
		}
		if ($this->_thumbnail->Visible && !$this->_thumbnail->Upload->KeepFile) {
			$this->_thumbnail->UploadPath = '../assets/images/model_thumbnail/';
		...
		if ($this->full_image->Visible && !$this->full_image->Upload->KeepFile) {
			...
			if (!ew_Empty($this->full_image->Upload->FileName)) {
				...
				$this->full_image->SetDbValueDef($rsnew, $this->full_image->Upload->FileName, NULL, FALSE);
			}
		}
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("model_color", $DetailTblVar) && $GLOBALS["model_color"]->DetailAdd) {
				$GLOBALS["model_color"]->model_id->setSessionValue($this->model_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["model_color_grid"])) $GLOBALS["model_color_grid"] = new cmodel_color_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "model_color"); // Load user level of detail table
				$AddRow = $GLOBALS["model_color_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["model_color"]->model_id->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("model_gallery", $DetailTblVar) && $GLOBALS["model_gallery"]->DetailAdd) {
				$GLOBALS["model_gallery"]->model_id->setSessionValue($this->model_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["model_gallery_grid"])) $GLOBALS["model_gallery_grid"] = new cmodel_gallery_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "model_gallery"); // Load user level of detail table
				$AddRow = $GLOBALS["model_gallery_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["model_gallery"]->model_id->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("feature", $DetailTblVar) && $GLOBALS["feature"]->DetailAdd) {
				$GLOBALS["feature"]->model_id->setSessionValue($this->model_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["feature_grid"])) $GLOBALS["feature_grid"] = new cfeature_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "feature"); // Load user level of detail table
				$AddRow = $GLOBALS["feature_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["feature"]->model_id->setSessionValue(""); // Clear master key if insert failed
			}
			if (in_array("specification", $DetailTblVar) && $GLOBALS["specification"]->DetailAdd) {
				$GLOBALS["specification"]->model_id->setSessionValue($this->model_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["specification_grid"])) $GLOBALS["specification_grid"] = new cspecification_grid(); // Get detail page object
				$Security->LoadCurrentUserLevel($this->ProjectID . "specification"); // Load user level of detail table
				$AddRow = $GLOBALS["specification_grid"]->GridInsert();
				$Security->LoadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
				if (!$AddRow)
					$GLOBALS["specification"]->model_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("model_color", $DetailTblVar)) {
				...
				if ($GLOBALS["model_color_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["model_color_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["model_color_grid"]->CurrentMode = "add";
					$GLOBALS["model_color_grid"]->CurrentAction = "gridadd";
			...
			if (in_array("model_gallery", $DetailTblVar)) {
				...
				if ($GLOBALS["model_gallery_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["model_gallery_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["model_gallery_grid"]->CurrentMode = "add";
					$GLOBALS["model_gallery_grid"]->CurrentAction = "gridadd";
			...
			if (in_array("feature", $DetailTblVar)) {
				...
				if ($GLOBALS["feature_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["feature_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["feature_grid"]->CurrentMode = "add";
					$GLOBALS["feature_grid"]->CurrentAction = "gridadd";
			...
			if (in_array("specification", $DetailTblVar)) {
				...
				if ($GLOBALS["specification_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["specification_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["specification_grid"]->CurrentMode = "add";
					$GLOBALS["specification_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($model_add)) $model_add = new cmodel_add();
...
var CurrentForm = fmodeladd = new ew_Form("fmodeladd", "add");

// Validate form
fmodeladd.Validate = function() {
...
fmodeladd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fmodeladd.Lists["x_category_id"] = {"LinkField":"x_category_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_category_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"model_category"};
fmodeladd.Lists["x_category_id"].Data = "<?php echo $model_add->category_id->LookupFilterQuery(FALSE, "add") ?>";
fmodeladd.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmodeladd.Lists["x_status"].Options = <?php echo json_encode($model_add->status->Options()) ?>;
fmodeladd.Lists["x_is_feature"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmodeladd.Lists["x_is_feature"].Options = <?php echo json_encode($model_add->is_feature->Options()) ?>;
fmodeladd.Lists["x_is_available"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmodeladd.Lists["x_is_available"].Options = <?php echo json_encode($model_add->is_available->Options()) ?>;
...
?>
<form name="fmodeladd" id="fmodeladd" class="<?php echo $model_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($model_add->CheckToken) { ?>
...
<?php if ($model->model_name->Visible) { // model_name ?>
	<div id="r_model_name" class="form-group">
		<label id="elh_model_model_name" for="x_model_name" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->model_name->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->model_name->CellAttributes() ?>>
...
<?php if ($model->price->Visible) { // price ?>
	<div id="r_price" class="form-group">
		<label id="elh_model_price" for="x_price" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->price->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->price->CellAttributes() ?>>
...
<?php if ($model->displacement->Visible) { // displacement ?>
	<div id="r_displacement" class="form-group">
		<label id="elh_model_displacement" for="x_displacement" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->displacement->FldCaption() ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->displacement->CellAttributes() ?>>
...
<?php if ($model->url_slug->Visible) { // url_slug ?>
	<div id="r_url_slug" class="form-group">
		<label id="elh_model_url_slug" for="x_url_slug" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->url_slug->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->url_slug->CellAttributes() ?>>
...
<?php if ($model->model_logo->Visible) { // model_logo ?>
	<div id="r_model_logo" class="form-group">
		<label id="elh_model_model_logo" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->model_logo->FldCaption() ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->model_logo->CellAttributes() ?>>
...
<?php if ($model->model_year->Visible) { // model_year ?>
	<div id="r_model_year" class="form-group">
		<label id="elh_model_model_year" for="x_model_year" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->model_year->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->model_year->CellAttributes() ?>>
...
<?php if ($model->icon_menu->Visible) { // icon_menu ?>
	<div id="r_icon_menu" class="form-group">
		<label id="elh_model_icon_menu" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->icon_menu->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->icon_menu->CellAttributes() ?>>
...
<?php if ($model->_thumbnail->Visible) { // thumbnail ?>
	<div id="r__thumbnail" class="form-group">
		<label id="elh_model__thumbnail" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->_thumbnail->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->_thumbnail->CellAttributes() ?>>
...
<?php if ($model->full_image->Visible) { // full_image ?>
	<div id="r_full_image" class="form-group">
		<label id="elh_model_full_image" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->full_image->FldCaption() ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->full_image->CellAttributes() ?>>
...
<?php if ($model->description->Visible) { // description ?>
	<div id="r_description" class="form-group">
		<label id="elh_model_description" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->description->FldCaption() ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->description->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateEditor("fmodeladd", "x_description", 45, 6, <?php echo ($model->description->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php if ($model->youtube_url->Visible) { // youtube_url ?>
	<div id="r_youtube_url" class="form-group">
		<label id="elh_model_youtube_url" for="x_youtube_url" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->youtube_url->FldCaption() ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->youtube_url->CellAttributes() ?>>
...
<?php if ($model->category_id->Visible) { // category_id ?>
	<div id="r_category_id" class="form-group">
		<label id="elh_model_category_id" for="x_category_id" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->category_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->category_id->CellAttributes() ?>>
...
<?php if ($model->m_order->Visible) { // m_order ?>
	<div id="r_m_order" class="form-group">
		<label id="elh_model_m_order" for="x_m_order" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->m_order->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->m_order->CellAttributes() ?>>
...
<?php if ($model->seo_description->Visible) { // seo_description ?>
	<div id="r_seo_description" class="form-group">
		<label id="elh_model_seo_description" for="x_seo_description" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->seo_description->FldCaption() ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->seo_description->CellAttributes() ?>>
...
<?php if ($model->seo_keyword->Visible) { // seo_keyword ?>
	<div id="r_seo_keyword" class="form-group">
		<label id="elh_model_seo_keyword" for="x_seo_keyword" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->seo_keyword->FldCaption() ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->seo_keyword->CellAttributes() ?>>
...
<?php if ($model->status->Visible) { // status ?>
	<div id="r_status" class="form-group">
		<label id="elh_model_status" for="x_status" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->status->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->status->CellAttributes() ?>>
...
<?php if ($model->is_feature->Visible) { // is_feature ?>
	<div id="r_is_feature" class="form-group">
		<label id="elh_model_is_feature" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->is_feature->FldCaption() ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->is_feature->CellAttributes() ?>>
...
<?php if ($model->is_available->Visible) { // is_available ?>
	<div id="r_is_available" class="form-group">
		<label id="elh_model_is_available" class="<?php echo $model_add->LeftColumnClass ?>"><?php echo $model->is_available->FldCaption() ?></label>
		<div class="<?php echo $model_add->RightColumnClass ?>"><div<?php echo $model->is_available->CellAttributes() ?>>
...
<?php
	if (in_array("model_color", explode(",", $model->getCurrentDetailTable())) && $model_color->DetailAdd) {
?>
...
<?php } ?>
<?php
	if (in_array("model_gallery", explode(",", $model->getCurrentDetailTable())) && $model_gallery->DetailAdd) {
?>
<?php if ($model->getCurrentDetailTable() <> "") { ?>
...
<?php } ?>
<?php
	if (in_array("feature", explode(",", $model->getCurrentDetailTable())) && $feature->DetailAdd) {
?>
<?php if ($model->getCurrentDetailTable() <> "") { ?>
...
<?php } ?>
<?php
	if (in_array("specification", explode(",", $model->getCurrentDetailTable())) && $specification->DetailAdd) {
?>
<?php if ($model->getCurrentDetailTable() <> "") { ?>
