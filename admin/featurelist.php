...
class cfeature_list extends cfeature {
	...
	function __construct() {
		...
		$this->FilterOptions->TagClassName = "ewFilterOption ffeaturelistsrch";
	...
	function GetFilterList() {
		...
		$sFilterList = "";
		$sSavedFilterList = "";
		$sFilterList = ew_Concat($sFilterList, $this->feature_id->AdvancedSearch->ToJson(), ","); // Field feature_id
		$sFilterList = ew_Concat($sFilterList, $this->top_title->AdvancedSearch->ToJson(), ","); // Field top_title
		...
		$sFilterList = ew_Concat($sFilterList, $this->f_order->AdvancedSearch->ToJson(), ","); // Field f_order
		if ($this->BasicSearch->Keyword <> "") {
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "ffeaturelistsrch", $filters);
	...
	function RestoreFilterList() {
		...
		// Field feature_id
		$this->feature_id->AdvancedSearch->SearchValue = @$filter["x_feature_id"];
		$this->feature_id->AdvancedSearch->SearchOperator = @$filter["z_feature_id"];
		$this->feature_id->AdvancedSearch->SearchCondition = @$filter["v_feature_id"];
		$this->feature_id->AdvancedSearch->SearchValue2 = @$filter["y_feature_id"];
		$this->feature_id->AdvancedSearch->SearchOperator2 = @$filter["w_feature_id"];
		$this->feature_id->AdvancedSearch->Save();

		// Field top_title
		$this->top_title->AdvancedSearch->SearchValue = @$filter["x_top_title"];
		$this->top_title->AdvancedSearch->SearchOperator = @$filter["z_top_title"];
		$this->top_title->AdvancedSearch->SearchCondition = @$filter["v_top_title"];
		$this->top_title->AdvancedSearch->SearchValue2 = @$filter["y_top_title"];
		$this->top_title->AdvancedSearch->SearchOperator2 = @$filter["w_top_title"];
		$this->top_title->AdvancedSearch->Save();
		...
		// Field f_order
		$this->f_order->AdvancedSearch->SearchValue = @$filter["x_f_order"];
		$this->f_order->AdvancedSearch->SearchOperator = @$filter["z_f_order"];
		$this->f_order->AdvancedSearch->SearchCondition = @$filter["v_f_order"];
		$this->f_order->AdvancedSearch->SearchValue2 = @$filter["y_f_order"];
		$this->f_order->AdvancedSearch->SearchOperator2 = @$filter["w_f_order"];
		$this->f_order->AdvancedSearch->Save();
	...
	function BasicSearchSQL($arKeywords, $type) {
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $this->top_title, $arKeywords, $type);
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" class=\"ewMultiSelect\" value=\"" . ew_HtmlEncode($this->feature_id->CurrentValue) . "\" onclick=\"ew_ClickMultiCheckbox(event);\">";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"ffeaturelistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"ffeaturelistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.ffeaturelist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"ffeaturelistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function RenderRow() {
		...
		$this->f_order->ViewCustomAttributes = "";
			...
			if ($this->_thumbnail->UseColorbox) {
				...
				$this->_thumbnail->LinkAttrs["data-rel"] = "feature_x" . $this->RowCnt . "__thumbnail";
...
var CurrentForm = ffeaturelist = new ew_Form("ffeaturelist", "list");
ffeaturelist.FormKeyCountName = '<?php echo $feature_list->FormKeyCountName ?>';
...
ffeaturelist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ffeaturelist.Lists["x_model_id"] = {"LinkField":"x_model_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_model_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"model"};
ffeaturelist.Lists["x_model_id"].Data = "<?php echo $feature_list->model_id->LookupFilterQuery(FALSE, "list") ?>";

// Form object for search
var CurrentSearchForm = ffeaturelistsrch = new ew_Form("ffeaturelistsrch");
</script>
<script type="text/javascript">
...
<div class="ewToolbar">
<?php if ($feature_list->TotalRecs > 0 && $feature_list->ExportOptions->Visible()) { ?>
<?php $feature_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($feature_list->SearchOptions->Visible()) { ?>
<?php $feature_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($feature_list->FilterOptions->Visible()) { ?>
<?php $feature_list->FilterOptions->Render("body") ?>
<?php } ?>
...
<?php } ?>
<?php
	$bSelectLimit = $feature_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($feature_list->TotalRecs <= 0)
			$feature_list->TotalRecs = $feature->ListRecordCount();
	} else {
		if (!$feature_list->Recordset && ($feature_list->Recordset = $feature_list->LoadRecordset()))
	...
	if ($feature->CurrentAction == "" && $feature_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$feature_list->setWarningMessage(ew_DeniedMsg());
		if ($feature_list->SearchWhere == "0=101")
			$feature_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$feature_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$feature_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($feature->Export == "" && $feature->CurrentAction == "") { ?>
<form name="ffeaturelistsrch" id="ffeaturelistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($feature_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="ffeaturelistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="feature">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($feature_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($feature_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $feature_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($feature_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($feature_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($feature_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($feature_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
?>
<?php if ($feature_list->TotalRecs > 0 || $feature->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($feature_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> feature">
<div class="box-header ewGridUpperPanel">
<?php if ($feature->CurrentAction <> "gridadd" && $feature->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($feature_list->Pager)) $feature_list->Pager = new cPrevNextPager($feature_list->StartRec, $feature_list->DisplayRecs, $feature_list->TotalRecs, $feature_list->AutoHidePager) ?>
<?php if ($feature_list->Pager->RecordCount > 0 && $feature_list->Pager->Visible) { ?>
<div class="ewPager">
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $feature_list->PageUrl() ?>start=<?php echo $feature_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($feature_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $feature_list->PageUrl() ?>start=<?php echo $feature_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $feature_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($feature_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $feature_list->PageUrl() ?>start=<?php echo $feature_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($feature_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $feature_list->PageUrl() ?>start=<?php echo $feature_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $feature_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($feature_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $feature_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $feature_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $feature_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($feature_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
</div>
<form name="ffeaturelist" id="ffeaturelist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($feature_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $feature_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="feature">
<?php if ($feature->getCurrentMasterTable() == "model" && $feature->CurrentAction <> "") { ?>
...
<?php if ($feature_list->TotalRecs > 0 || $feature->CurrentAction == "gridedit") { ?>
<table id="tbl_featurelist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$feature_list->RowType = EW_ROWTYPE_HEADER;
...
<?php if ($feature->feature_id->Visible) { // feature_id ?>
	...
	<?php } else { ?>
		<th data-name="feature_id" class="<?php echo $feature->feature_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $feature->SortUrl($feature->feature_id) ?>',1);"><div id="elh_feature_feature_id" class="feature_feature_id">
...
<?php if ($feature->top_title->Visible) { // top_title ?>
	...
	<?php } else { ?>
		<th data-name="top_title" class="<?php echo $feature->top_title->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $feature->SortUrl($feature->top_title) ?>',1);"><div id="elh_feature_top_title" class="feature_top_title">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $feature->top_title->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($feature->top_title->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($feature->top_title->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($feature->title->Visible) { // title ?>
	...
	<?php } else { ?>
		<th data-name="title" class="<?php echo $feature->title->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $feature->SortUrl($feature->title) ?>',1);"><div id="elh_feature_title" class="feature_title">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $feature->title->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($feature->title->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($feature->title->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($feature->model_id->Visible) { // model_id ?>
	...
	<?php } else { ?>
		<th data-name="model_id" class="<?php echo $feature->model_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $feature->SortUrl($feature->model_id) ?>',1);"><div id="elh_feature_model_id" class="feature_model_id">
...
<?php if ($feature->_thumbnail->Visible) { // thumbnail ?>
	...
	<?php } else { ?>
		<th data-name="_thumbnail" class="<?php echo $feature->_thumbnail->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $feature->SortUrl($feature->_thumbnail) ?>',1);"><div id="elh_feature__thumbnail" class="feature__thumbnail">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $feature->_thumbnail->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($feature->_thumbnail->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($feature->_thumbnail->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($feature->f_order->Visible) { // f_order ?>
	...
	<?php } else { ?>
		<th data-name="f_order" class="<?php echo $feature->f_order->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $feature->SortUrl($feature->f_order) ?>',1);"><div id="elh_feature_f_order" class="feature_f_order">
...
<span id="el<?php echo $feature_list->RowCnt ?>_feature_feature_id" class="feature_feature_id">
...
<span id="el<?php echo $feature_list->RowCnt ?>_feature_top_title" class="feature_top_title">
...
<span id="el<?php echo $feature_list->RowCnt ?>_feature_title" class="feature_title">
...
<span id="el<?php echo $feature_list->RowCnt ?>_feature_model_id" class="feature_model_id">
...
<span id="el<?php echo $feature_list->RowCnt ?>_feature__thumbnail" class="feature__thumbnail">
<span>
...
<span id="el<?php echo $feature_list->RowCnt ?>_feature_f_order" class="feature_f_order">
...
<?php } ?>
<?php if ($feature_list->TotalRecs == 0 && $feature->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($feature_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
ffeaturelistsrch.FilterList = <?php echo $feature_list->GetFilterList() ?>;
