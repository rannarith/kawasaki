	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["mc_id"] <> "") {
				...
				$this->mc_id->setFormValue($_POST["mc_id"]);
				$this->RecKey["mc_id"] = $this->mc_id->FormValue;
			} else {
...
var CurrentForm = fmodel_colorview = new ew_Form("fmodel_colorview", "view");
...
fmodel_colorview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fmodel_colorview.Lists["x_model_id"] = {"LinkField":"x_model_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_model_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"model"};
fmodel_colorview.Lists["x_model_id"].Data = "<?php echo $model_color_view->model_id->LookupFilterQuery(FALSE, "view") ?>";
fmodel_colorview.Lists["x_m_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmodel_colorview.Lists["x_m_status"].Options = <?php echo json_encode($model_color_view->m_status->Options()) ?>;
...
<div class="ewToolbar">
<?php $model_color_view->ExportOptions->Render("body") ?>
<?php
	foreach ($model_color_view->OtherOptions as &$option)
		$option->Render("body");
?>
...
?>
<form name="fmodel_colorview" id="fmodel_colorview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($model_color_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $model_color_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="model_color">
<input type="hidden" name="modal" value="<?php echo intval($model_color_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($model_color->mc_id->Visible) { // mc_id ?>
	<tr id="r_mc_id">
		<td class="col-sm-2"><span id="elh_model_color_mc_id"><?php echo $model_color->mc_id->FldCaption() ?></span></td>
...
<?php if ($model_color->image->Visible) { // image ?>
	<tr id="r_image">
		<td class="col-sm-2"><span id="elh_model_color_image"><?php echo $model_color->image->FldCaption() ?></span></td>
		<td data-name="image"<?php echo $model_color->image->CellAttributes() ?>>
<span id="el_model_color_image">
<span>
<?php echo ew_GetFileViewTag($model_color->image, $model_color->image->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($model_color->model_id->Visible) { // model_id ?>
	<tr id="r_model_id">
		<td class="col-sm-2"><span id="elh_model_color_model_id"><?php echo $model_color->model_id->FldCaption() ?></span></td>
...
<?php if ($model_color->color_code->Visible) { // color_code ?>
	<tr id="r_color_code">
		<td class="col-sm-2"><span id="elh_model_color_color_code"><?php echo $model_color->color_code->FldCaption() ?></span></td>
...
<?php if ($model_color->m_order->Visible) { // m_order ?>
	<tr id="r_m_order">
		<td class="col-sm-2"><span id="elh_model_color_m_order"><?php echo $model_color->m_order->FldCaption() ?></span></td>
...
<?php if ($model_color->m_status->Visible) { // m_status ?>
	<tr id="r_m_status">
		<td class="col-sm-2"><span id="elh_model_color_m_status"><?php echo $model_color->m_status->FldCaption() ?></span></td>
