...
class cnews_list extends cnews {
	...
	function GetFilterList() {
		...
		$sFilterList = ew_Concat($sFilterList, $this->news_date->AdvancedSearch->ToJson(), ","); // Field news_date
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fnewslistsrch", $filters);
	...
	function RestoreFilterList() {
		...
		// Field news_date
		$this->news_date->AdvancedSearch->SearchValue = @$filter["x_news_date"];
		$this->news_date->AdvancedSearch->SearchOperator = @$filter["z_news_date"];
		$this->news_date->AdvancedSearch->SearchCondition = @$filter["v_news_date"];
		$this->news_date->AdvancedSearch->SearchValue2 = @$filter["y_news_date"];
		$this->news_date->AdvancedSearch->SearchOperator2 = @$filter["w_news_date"];
		$this->news_date->AdvancedSearch->Save();
	...
	function BasicSearchSQL($arKeywords, $type) {
		...
		$this->BuildBasicSearchSQL($sWhere, $this->news_date, $arKeywords, $type);
		return $sWhere;
	}
	...
	function SetupListOptions() {
		...
		$item->CssClass = "text-nowrap";
		$item->Visible = $Security->AllowList(CurrentProjectID() . 'news_gallery') && !$this->ShowMultipleDetails;
		$item->OnLeft = FALSE;
	...
	function RenderListOptions() {
		...
		if ($Security->AllowList(CurrentProjectID() . 'news_gallery')) {
			$body = $Language->Phrase("DetailLink") . $Language->TablePhrase("news_gallery", "TblCaption");
			...
			$links = "";
			if ($GLOBALS["news_gallery_grid"]->DetailView && $Security->CanView() && $Security->AllowView(CurrentProjectID() . 'news_gallery')) {
				$caption = $Language->Phrase("MasterDetailViewLink");
				$url = $this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=news_gallery");
			...
			if ($GLOBALS["news_gallery_grid"]->DetailEdit && $Security->CanEdit() && $Security->AllowEdit(CurrentProjectID() . 'news_gallery')) {
				$caption = $Language->Phrase("MasterDetailEditLink");
				$url = $this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=news_gallery");
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" class=\"ewMultiSelect\" value=\"" . ew_HtmlEncode($this->news_id->CurrentValue) . "\" onclick=\"ew_ClickMultiCheckbox(event);\">";
	...
	function SetupOtherOptions() {
		...
		$DetailTableLink = "";
		$item = &$option->Add("detailadd_news_gallery");
		$url = $this->GetAddUrl(EW_TABLE_SHOW_DETAIL . "=news_gallery");
		$caption = $Language->Phrase("Add") . "&nbsp;" . $this->TableCaption() . "/" . $GLOBALS["news_gallery"]->TableCaption();
		$item->Body = "<a class=\"ewDetailAddGroup ewDetailAdd\" title=\"" . ew_HtmlTitle($caption) . "\" data-caption=\"" . ew_HtmlTitle($caption) . "\" href=\"" . ew_HtmlEncode($url) . "\">" . $caption . "</a>";
		$item->Visible = ($GLOBALS["news_gallery"]->DetailAdd && $Security->AllowAdd(CurrentProjectID() . 'news_gallery') && $Security->CanAdd());
		if ($item->Visible) {
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fnewslistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function RenderRow() {
		...
		$this->status->ViewCustomAttributes = "";
			...
			if ($this->_thumbnail->UseColorbox) {
				...
				$this->_thumbnail->LinkAttrs["data-rel"] = "news_x" . $this->RowCnt . "__thumbnail";
...
fnewslist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fnewslist.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fnewslist.Lists["x_status"].Options = <?php echo json_encode($news_list->status->Options()) ?>;

// Form object for search
var CurrentSearchForm = fnewslistsrch = new ew_Form("fnewslistsrch");
</script>
<script type="text/javascript">
...
<?php
	...
	if ($bSelectLimit) {
		if ($news_list->TotalRecs <= 0)
			$news_list->TotalRecs = $news->ListRecordCount();
	} else {
	...
	if ($news->CurrentAction == "" && $news_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$news_list->setWarningMessage(ew_DeniedMsg());
...
<form name="fnewslistsrch" id="fnewslistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($news_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fnewslistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="news">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($news_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($news_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $news_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($news_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($news_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($news_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($news_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<div class="box ewBox ewGrid<?php if ($news_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> news">
<div class="box-header ewGridUpperPanel">
<?php if ($news->CurrentAction <> "gridadd" && $news->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($news_list->Pager)) $news_list->Pager = new cPrevNextPager($news_list->StartRec, $news_list->DisplayRecs, $news_list->TotalRecs, $news_list->AutoHidePager) ?>
...
<?php } ?>
<input type="hidden" name="t" value="news">
<div id="gmp_news" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($news_list->TotalRecs > 0 || $news->CurrentAction == "gridedit") { ?>
...
<?php if ($news->news_id->Visible) { // news_id ?>
	<?php if ($news->SortUrl($news->news_id) == "") { ?>
		<th data-name="news_id" class="<?php echo $news->news_id->HeaderCellClass() ?>"><div id="elh_news_news_id" class="news_news_id"><div class="ewTableHeaderCaption"><?php echo $news->news_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="news_id" class="<?php echo $news->news_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $news->SortUrl($news->news_id) ?>',1);"><div id="elh_news_news_id" class="news_news_id">
...
<?php if ($news->_thumbnail->Visible) { // thumbnail ?>
	<?php if ($news->SortUrl($news->_thumbnail) == "") { ?>
		<th data-name="_thumbnail" class="<?php echo $news->_thumbnail->HeaderCellClass() ?>"><div id="elh_news__thumbnail" class="news__thumbnail"><div class="ewTableHeaderCaption"><?php echo $news->_thumbnail->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="_thumbnail" class="<?php echo $news->_thumbnail->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $news->SortUrl($news->_thumbnail) ?>',1);"><div id="elh_news__thumbnail" class="news__thumbnail">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $news->_thumbnail->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($news->_thumbnail->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($news->_thumbnail->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($news->title->Visible) { // title ?>
	<?php if ($news->SortUrl($news->title) == "") { ?>
		<th data-name="title" class="<?php echo $news->title->HeaderCellClass() ?>"><div id="elh_news_title" class="news_title"><div class="ewTableHeaderCaption"><?php echo $news->title->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="title" class="<?php echo $news->title->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $news->SortUrl($news->title) ?>',1);"><div id="elh_news_title" class="news_title">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $news->title->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($news->title->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($news->title->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($news->youtube_url->Visible) { // youtube_url ?>
	<?php if ($news->SortUrl($news->youtube_url) == "") { ?>
		<th data-name="youtube_url" class="<?php echo $news->youtube_url->HeaderCellClass() ?>"><div id="elh_news_youtube_url" class="news_youtube_url"><div class="ewTableHeaderCaption"><?php echo $news->youtube_url->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="youtube_url" class="<?php echo $news->youtube_url->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $news->SortUrl($news->youtube_url) ?>',1);"><div id="elh_news_youtube_url" class="news_youtube_url">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $news->youtube_url->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($news->youtube_url->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($news->youtube_url->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($news->news_date->Visible) { // news_date ?>
	<?php if ($news->SortUrl($news->news_date) == "") { ?>
		<th data-name="news_date" class="<?php echo $news->news_date->HeaderCellClass() ?>"><div id="elh_news_news_date" class="news_news_date"><div class="ewTableHeaderCaption"><?php echo $news->news_date->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="news_date" class="<?php echo $news->news_date->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $news->SortUrl($news->news_date) ?>',1);"><div id="elh_news_news_date" class="news_news_date">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $news->news_date->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($news->news_date->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($news->news_date->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($news->status->Visible) { // status ?>
	<?php if ($news->SortUrl($news->status) == "") { ?>
		<th data-name="status" class="<?php echo $news->status->HeaderCellClass() ?>"><div id="elh_news_status" class="news_status"><div class="ewTableHeaderCaption"><?php echo $news->status->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status" class="<?php echo $news->status->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $news->SortUrl($news->status) ?>',1);"><div id="elh_news_status" class="news_status">
...
<span id="el<?php echo $news_list->RowCnt ?>_news__thumbnail" class="news__thumbnail">
<span>
...
<span id="el<?php echo $news_list->RowCnt ?>_news_youtube_url" class="news_youtube_url">
...
<span id="el<?php echo $news_list->RowCnt ?>_news_status" class="news_status">
