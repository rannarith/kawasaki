	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["contact_id"] <> "") {
				...
				$this->contact_id->setFormValue($_POST["contact_id"]);
				$this->RecKey["contact_id"] = $this->contact_id->FormValue;
			} else {
...
var CurrentForm = fcontactview = new ew_Form("fcontactview", "view");
...
fcontactview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<div class="ewToolbar">
<?php $contact_view->ExportOptions->Render("body") ?>
<?php
	foreach ($contact_view->OtherOptions as &$option)
		$option->Render("body");
?>
...
?>
<form name="fcontactview" id="fcontactview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($contact_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $contact_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="contact">
<input type="hidden" name="modal" value="<?php echo intval($contact_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($contact->contact_id->Visible) { // contact_id ?>
	<tr id="r_contact_id">
		<td class="col-sm-2"><span id="elh_contact_contact_id"><?php echo $contact->contact_id->FldCaption() ?></span></td>
...
<?php if ($contact->full_name->Visible) { // full_name ?>
	<tr id="r_full_name">
		<td class="col-sm-2"><span id="elh_contact_full_name"><?php echo $contact->full_name->FldCaption() ?></span></td>
...
<?php if ($contact->phone->Visible) { // phone ?>
	<tr id="r_phone">
		<td class="col-sm-2"><span id="elh_contact_phone"><?php echo $contact->phone->FldCaption() ?></span></td>
...
<?php echo $contact->phone->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($contact->_email->Visible) { // email ?>
	<tr id="r__email">
		<td class="col-sm-2"><span id="elh_contact__email"><?php echo $contact->_email->FldCaption() ?></span></td>
...
<?php if ($contact->message->Visible) { // message ?>
	<tr id="r_message">
		<td class="col-sm-2"><span id="elh_contact_message"><?php echo $contact->message->FldCaption() ?></span></td>
		<td data-name="message"<?php echo $contact->message->CellAttributes() ?>>
...
<?php if ($contact->send_date->Visible) { // send_date ?>
	<tr id="r_send_date">
		<td class="col-sm-2"><span id="elh_contact_send_date"><?php echo $contact->send_date->FldCaption() ?></span></td>
