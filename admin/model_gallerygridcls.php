...
$model_gallery_grid = NULL; // Initialize page object first

class cmodel_gallery_grid extends cmodel_gallery {
	...
	// Page object name
	var $PageObjName = 'model_gallery_grid';

	// Grid form hidden field names
	var $FormName = 'fmodel_gallerygrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["model_gallery"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->gallery_id->CurrentValue;
	...
	function EmptyRow() {
		...
		if (!ew_Empty($this->image->Upload->Value))
			return FALSE;
		if ($objForm->HasValue("x_img_order") && $objForm->HasValue("o_img_order") && $this->img_order->CurrentValue <> $this->img_order->OldValue)
			return FALSE;
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->gallery_id->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		if (!$this->gallery_id->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
		...
		$this->img_order->setOldValue($objForm->GetValue("o_img_order"));
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->gallery_id->CurrentValue = strval($arKeys[0]); // gallery_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
