...
class cmodel_gallery_view extends cmodel_gallery {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			} elseif (@$_POST["gallery_id"] <> "") {
				$this->gallery_id->setFormValue($_POST["gallery_id"]);
				$this->RecKey["gallery_id"] = $this->gallery_id->FormValue;
			} else {
...
$model_gallery_view->Page_Render();
?>
...
var CurrentForm = fmodel_galleryview = new ew_Form("fmodel_galleryview", "view");
...
fmodel_galleryview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fmodel_galleryview.Lists["x_model_id"] = {"LinkField":"x_model_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_model_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"model"};
fmodel_galleryview.Lists["x_model_id"].Data = "<?php echo $model_gallery_view->model_id->LookupFilterQuery(FALSE, "view") ?>";
fmodel_galleryview.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fmodel_galleryview.Lists["x_status"].Options = <?php echo json_encode($model_gallery_view->status->Options()) ?>;
...
<div class="ewToolbar">
<?php $model_gallery_view->ExportOptions->Render("body") ?>
<?php
	foreach ($model_gallery_view->OtherOptions as &$option)
		$option->Render("body");
?>
...
?>
<form name="fmodel_galleryview" id="fmodel_galleryview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($model_gallery_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $model_gallery_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="model_gallery">
<input type="hidden" name="modal" value="<?php echo intval($model_gallery_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($model_gallery->gallery_id->Visible) { // gallery_id ?>
	<tr id="r_gallery_id">
		<td class="col-sm-2"><span id="elh_model_gallery_gallery_id"><?php echo $model_gallery->gallery_id->FldCaption() ?></span></td>
...
<?php if ($model_gallery->model_id->Visible) { // model_id ?>
	<tr id="r_model_id">
		<td class="col-sm-2"><span id="elh_model_gallery_model_id"><?php echo $model_gallery->model_id->FldCaption() ?></span></td>
...
<?php if ($model_gallery->image->Visible) { // image ?>
	<tr id="r_image">
		<td class="col-sm-2"><span id="elh_model_gallery_image"><?php echo $model_gallery->image->FldCaption() ?></span></td>
		<td data-name="image"<?php echo $model_gallery->image->CellAttributes() ?>>
<span id="el_model_gallery_image">
<span>
<?php echo ew_GetFileViewTag($model_gallery->image, $model_gallery->image->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($model_gallery->img_order->Visible) { // img_order ?>
	<tr id="r_img_order">
		<td class="col-sm-2"><span id="elh_model_gallery_img_order"><?php echo $model_gallery->img_order->FldCaption() ?></span></td>
...
<?php if ($model_gallery->status->Visible) { // status ?>
	<tr id="r_status">
		<td class="col-sm-2"><span id="elh_model_gallery_status"><?php echo $model_gallery->status->FldCaption() ?></span></td>
