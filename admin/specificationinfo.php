...
class cspecification extends cTable {
	...
	function __construct() {
		...
		// Update Table
		$this->UpdateTable = "`specification`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// spec_id
		$this->spec_id = new cField('specification', 'specification', 'x_spec_id', 'spec_id', '`spec_id`', '`spec_id`', 3, -1, FALSE, '`spec_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->spec_id->Sortable = TRUE; // Allow sort
		$this->spec_id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['spec_id'] = &$this->spec_id;

		// model_id
		$this->model_id = new cField('specification', 'specification', 'x_model_id', 'model_id', '`model_id`', '`model_id`', 3, -1, FALSE, '`model_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		// title
		$this->title = new cField('specification', 'specification', 'x_title', 'title', '`title`', '`title`', 200, -1, FALSE, '`title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->title->Sortable = TRUE; // Allow sort
		$this->fields['title'] = &$this->title;

		// description
		$this->description = new cField('specification', 'specification', 'x_description', 'description', '`description`', '`description`', 201, -1, FALSE, '`description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->description->Sortable = TRUE; // Allow sort
		$this->fields['description'] = &$this->description;

		// s_order
		$this->s_order = new cField('specification', 'specification', 'x_s_order', 's_order', '`s_order`', '`s_order`', 16, -1, FALSE, '`s_order`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// status
		$this->status = new cField('specification', 'specification', 'x_status', 'status', '`status`', '`status`', 16, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`specification`";
	}
	...
	function getSqlOrderBy() { // Order By
		return ($this->_SqlOrderBy <> "") ? $this->_SqlOrderBy : "`spec_id` DESC";
	}
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$rs['spec_id'] = $this->spec_id->DbValue;
		}
		return $bInsert;
	}
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if (is_array($where))
			$where = $this->ArrayToFilter($where);
		if ($rs) {
			if (array_key_exists('spec_id', $rs))
				ew_AddFilter($where, ew_QuotedName('spec_id', $this->DBID) . '=' . ew_QuotedValue($rs['spec_id'], $this->spec_id->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		if (is_null($this->spec_id->CurrentValue))
			...
			$sKeyFilter = str_replace("@spec_id@", ew_AdjustSql($this->spec_id->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetModalCaption($pageName) {
		global $Language;
		if ($pageName == "specificationview.php")
			return $Language->Phrase("View");
		elseif ($pageName == "specificationedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "specificationadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("specificationview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("specificationview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "specificationadd.php?" . $this->UrlParm($parm);
		else
			$url = "specificationadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("specificationedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("specificationadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "spec_id:" . ew_VarToJson($this->spec_id->CurrentValue, "number", "'");
		return "{" . $json . "}";
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$isPost = ew_IsPost();
			if ($isPost && isset($_POST["spec_id"]))
				$arKeys[] = $_POST["spec_id"];
	...
	function RenderEditRow() {
		...
		// spec_id
		$this->spec_id->EditAttrs["class"] = "form-control";
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					if ($this->spec_id->Exportable) $Doc->ExportCaption($this->spec_id);
		...
		while (!$Recordset->EOF && $RecCnt < $StopRec) {
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						if ($this->spec_id->Exportable) $Doc->ExportField($this->spec_id);
