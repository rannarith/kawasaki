...
class cspecification_list extends cspecification {
	...
	var $FormName = 'fspecificationlist';
	...
	function __construct() {
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fspecificationlistsrch";
	...
	function GetFilterList() {
		...
		$sFilterList = "";
		$sSavedFilterList = "";
		$sFilterList = ew_Concat($sFilterList, $this->spec_id->AdvancedSearch->ToJson(), ","); // Field spec_id
		...
		$sFilterList = ew_Concat($sFilterList, $this->s_order->AdvancedSearch->ToJson(), ","); // Field s_order
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fspecificationlistsrch", $filters);
	...
	function RestoreFilterList() {
		...
		// Field spec_id
		$this->spec_id->AdvancedSearch->SearchValue = @$filter["x_spec_id"];
		$this->spec_id->AdvancedSearch->SearchOperator = @$filter["z_spec_id"];
		$this->spec_id->AdvancedSearch->SearchCondition = @$filter["v_spec_id"];
		$this->spec_id->AdvancedSearch->SearchValue2 = @$filter["y_spec_id"];
		$this->spec_id->AdvancedSearch->SearchOperator2 = @$filter["w_spec_id"];
		$this->spec_id->AdvancedSearch->Save();
		...
		// Field s_order
		$this->s_order->AdvancedSearch->SearchValue = @$filter["x_s_order"];
		$this->s_order->AdvancedSearch->SearchOperator = @$filter["z_s_order"];
		$this->s_order->AdvancedSearch->SearchCondition = @$filter["v_s_order"];
		$this->s_order->AdvancedSearch->SearchValue2 = @$filter["y_s_order"];
		$this->s_order->AdvancedSearch->SearchOperator2 = @$filter["w_s_order"];
		$this->s_order->AdvancedSearch->Save();
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<input type=\"checkbox\" name=\"key_m[]\" class=\"ewMultiSelect\" value=\"" . ew_HtmlEncode($this->spec_id->CurrentValue) . "\" onclick=\"ew_ClickMultiCheckbox(event);\">";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fspecificationlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fspecificationlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fspecificationlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fspecificationlistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
...
$specification_list->Page_Render();
?>
...
var CurrentForm = fspecificationlist = new ew_Form("fspecificationlist", "list");
fspecificationlist.FormKeyCountName = '<?php echo $specification_list->FormKeyCountName ?>';
...
fspecificationlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fspecificationlist.Lists["x_model_id"] = {"LinkField":"x_model_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_model_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"model"};
fspecificationlist.Lists["x_model_id"].Data = "<?php echo $specification_list->model_id->LookupFilterQuery(FALSE, "list") ?>";
fspecificationlist.Lists["x_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fspecificationlist.Lists["x_status"].Options = <?php echo json_encode($specification_list->status->Options()) ?>;

// Form object for search
var CurrentSearchForm = fspecificationlistsrch = new ew_Form("fspecificationlistsrch");
</script>
<script type="text/javascript">
...
<div class="ewToolbar">
<?php if ($specification_list->TotalRecs > 0 && $specification_list->ExportOptions->Visible()) { ?>
<?php $specification_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($specification_list->SearchOptions->Visible()) { ?>
<?php $specification_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($specification_list->FilterOptions->Visible()) { ?>
<?php $specification_list->FilterOptions->Render("body") ?>
<?php } ?>
...
<?php } ?>
<?php
	$bSelectLimit = $specification_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($specification_list->TotalRecs <= 0)
			$specification_list->TotalRecs = $specification->ListRecordCount();
	} else {
		if (!$specification_list->Recordset && ($specification_list->Recordset = $specification_list->LoadRecordset()))
	...
	if ($specification->CurrentAction == "" && $specification_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$specification_list->setWarningMessage(ew_DeniedMsg());
		if ($specification_list->SearchWhere == "0=101")
			$specification_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$specification_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$specification_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($specification->Export == "" && $specification->CurrentAction == "") { ?>
<form name="fspecificationlistsrch" id="fspecificationlistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($specification_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fspecificationlistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="specification">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($specification_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($specification_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $specification_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($specification_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($specification_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($specification_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($specification_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
?>
<?php if ($specification_list->TotalRecs > 0 || $specification->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($specification_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> specification">
<div class="box-header ewGridUpperPanel">
<?php if ($specification->CurrentAction <> "gridadd" && $specification->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($specification_list->Pager)) $specification_list->Pager = new cPrevNextPager($specification_list->StartRec, $specification_list->DisplayRecs, $specification_list->TotalRecs, $specification_list->AutoHidePager) ?>
<?php if ($specification_list->Pager->RecordCount > 0 && $specification_list->Pager->Visible) { ?>
<div class="ewPager">
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $specification_list->PageUrl() ?>start=<?php echo $specification_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($specification_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $specification_list->PageUrl() ?>start=<?php echo $specification_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $specification_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($specification_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $specification_list->PageUrl() ?>start=<?php echo $specification_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($specification_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $specification_list->PageUrl() ?>start=<?php echo $specification_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $specification_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($specification_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $specification_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $specification_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $specification_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($specification_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
</div>
<form name="fspecificationlist" id="fspecificationlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($specification_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $specification_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="specification">
<?php if ($specification->getCurrentMasterTable() == "model" && $specification->CurrentAction <> "") { ?>
<input type="hidden" name="<?php echo EW_TABLE_SHOW_MASTER ?>" value="model">
<input type="hidden" name="fk_model_id" value="<?php echo $specification->model_id->getSessionValue() ?>">
<?php } ?>
<div id="gmp_specification" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($specification_list->TotalRecs > 0 || $specification->CurrentAction == "gridedit") { ?>
<table id="tbl_specificationlist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$specification_list->RowType = EW_ROWTYPE_HEADER;
...
<?php if ($specification->spec_id->Visible) { // spec_id ?>
	...
	<?php } else { ?>
		<th data-name="spec_id" class="<?php echo $specification->spec_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $specification->SortUrl($specification->spec_id) ?>',1);"><div id="elh_specification_spec_id" class="specification_spec_id">
...
<?php if ($specification->model_id->Visible) { // model_id ?>
	...
	<?php } else { ?>
		<th data-name="model_id" class="<?php echo $specification->model_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $specification->SortUrl($specification->model_id) ?>',1);"><div id="elh_specification_model_id" class="specification_model_id">
...
<?php if ($specification->title->Visible) { // title ?>
	...
	<?php } else { ?>
		<th data-name="title" class="<?php echo $specification->title->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $specification->SortUrl($specification->title) ?>',1);"><div id="elh_specification_title" class="specification_title">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $specification->title->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($specification->title->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($specification->title->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($specification->s_order->Visible) { // s_order ?>
	...
	<?php } else { ?>
		<th data-name="s_order" class="<?php echo $specification->s_order->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $specification->SortUrl($specification->s_order) ?>',1);"><div id="elh_specification_s_order" class="specification_s_order">
...
<?php if ($specification->status->Visible) { // status ?>
	...
	<?php } else { ?>
		<th data-name="status" class="<?php echo $specification->status->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $specification->SortUrl($specification->status) ?>',1);"><div id="elh_specification_status" class="specification_status">
...
<span id="el<?php echo $specification_list->RowCnt ?>_specification_spec_id" class="specification_spec_id">
...
<span id="el<?php echo $specification_list->RowCnt ?>_specification_model_id" class="specification_model_id">
...
<span id="el<?php echo $specification_list->RowCnt ?>_specification_title" class="specification_title">
...
<span id="el<?php echo $specification_list->RowCnt ?>_specification_s_order" class="specification_s_order">
...
<span id="el<?php echo $specification_list->RowCnt ?>_specification_status" class="specification_status">
...
<?php } ?>
<?php if ($specification_list->TotalRecs == 0 && $specification->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($specification_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
fspecificationlistsrch.FilterList = <?php echo $specification_list->GetFilterList() ?>;
