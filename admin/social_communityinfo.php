...
$social_community = NULL;

//
// Table class for social_community
//
class csocial_community extends cTable {
	var $id;
	var $_thumbnail;
	var $little;
	var $linkto;
	...
	function __construct() {
		...
		$this->TableVar = 'social_community';
		$this->TableName = 'social_community';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`social_community`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('social_community', 'social_community', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// thumbnail
		$this->_thumbnail = new cField('social_community', 'social_community', 'x__thumbnail', 'thumbnail', '`thumbnail`', '`thumbnail`', 200, -1, TRUE, '`thumbnail`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->_thumbnail->Sortable = TRUE; // Allow sort
		$this->fields['thumbnail'] = &$this->_thumbnail;

		// little
		$this->little = new cField('social_community', 'social_community', 'x_little', 'little', '`little`', '`little`', 200, -1, FALSE, '`little`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->little->Sortable = TRUE; // Allow sort
		$this->fields['little'] = &$this->little;

		// linkto
		$this->linkto = new cField('social_community', 'social_community', 'x_linkto', 'linkto', '`linkto`', '`linkto`', 200, -1, FALSE, '`linkto`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->linkto->Sortable = TRUE; // Allow sort
		$this->fields['linkto'] = &$this->linkto;

		// social_type
		$this->social_type = new cField('social_community', 'social_community', 'x_social_type', 'social_type', '`social_type`', '`social_type`', 200, -1, FALSE, '`social_type`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		$this->social_type->Sortable = TRUE; // Allow sort
		$this->social_type->UsePleaseSelect = TRUE; // Use PleaseSelect by default
		$this->social_type->PleaseSelectText = $Language->Phrase("PleaseSelect"); // PleaseSelect text
		$this->social_type->OptionCount = 3;
		$this->fields['social_type'] = &$this->social_type;

		// create_date
		$this->create_date = new cField('social_community', 'social_community', 'x_create_date', 'create_date', '`create_date`', ew_CastDateFieldForLike('`create_date`', 0, "DB"), 135, 0, FALSE, '`create_date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// is_active
		$this->is_active = new cField('social_community', 'social_community', 'x_is_active', 'is_active', '`is_active`', '`is_active`', 16, -1, FALSE, '`is_active`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`social_community`";
	}
	...
	function getReturnUrl() {
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "social_communitylist.php";
		}
	}
	...
	function GetModalCaption($pageName) {
		...
		elseif ($pageName == "social_communityedit.php")
			return $Language->Phrase("Edit");
		elseif ($pageName == "social_communityadd.php")
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("social_communityview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("social_communityview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "social_communityadd.php?" . $this->UrlParm($parm);
		else
			$url = "social_communityadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("social_communityedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("social_communityadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("social_communitydelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->little->setDbValue($rs->fields('little'));
		$this->linkto->setDbValue($rs->fields('linkto'));
		$this->social_type->setDbValue($rs->fields('social_type'));
	...
	function RenderEditRow() {
		...
		$this->little->EditValue = $this->little->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->little->Exportable) $Doc->ExportCaption($this->little);
					if ($this->linkto->Exportable) $Doc->ExportCaption($this->linkto);
					if ($this->social_type->Exportable) $Doc->ExportCaption($this->social_type);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->little->Exportable) $Doc->ExportField($this->little);
						if ($this->linkto->Exportable) $Doc->ExportField($this->linkto);
						if ($this->social_type->Exportable) $Doc->ExportField($this->social_type);
