	...
	function Page_Main() {
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("contactlist.php"); // Return to list
			}
		}
	}
...
var CurrentForm = fcontactdelete = new ew_Form("fcontactdelete", "delete");
...
fcontactdelete.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
?>
<form name="fcontactdelete" id="fcontactdelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($contact_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $contact_delete->Token ?>">
<?php } ?>
...
<?php if ($contact->contact_id->Visible) { // contact_id ?>
		<th class="<?php echo $contact->contact_id->HeaderCellClass() ?>"><span id="elh_contact_contact_id" class="contact_contact_id"><?php echo $contact->contact_id->FldCaption() ?></span></th>
<?php } ?>
<?php if ($contact->full_name->Visible) { // full_name ?>
		<th class="<?php echo $contact->full_name->HeaderCellClass() ?>"><span id="elh_contact_full_name" class="contact_full_name"><?php echo $contact->full_name->FldCaption() ?></span></th>
<?php } ?>
<?php if ($contact->phone->Visible) { // phone ?>
		<th class="<?php echo $contact->phone->HeaderCellClass() ?>"><span id="elh_contact_phone" class="contact_phone"><?php echo $contact->phone->FldCaption() ?></span></th>
<?php } ?>
<?php if ($contact->_email->Visible) { // email ?>
		<th class="<?php echo $contact->_email->HeaderCellClass() ?>"><span id="elh_contact__email" class="contact__email"><?php echo $contact->_email->FldCaption() ?></span></th>
<?php } ?>
<?php if ($contact->send_date->Visible) { // send_date ?>
		<th class="<?php echo $contact->send_date->HeaderCellClass() ?>"><span id="elh_contact_send_date" class="contact_send_date"><?php echo $contact->send_date->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
...
<?php if ($contact->contact_id->Visible) { // contact_id ?>
		<td<?php echo $contact->contact_id->CellAttributes() ?>>
<span id="el<?php echo $contact_delete->RowCnt ?>_contact_contact_id" class="contact_contact_id">
...
<?php } ?>
<?php if ($contact->full_name->Visible) { // full_name ?>
		<td<?php echo $contact->full_name->CellAttributes() ?>>
<span id="el<?php echo $contact_delete->RowCnt ?>_contact_full_name" class="contact_full_name">
...
<?php } ?>
<?php if ($contact->phone->Visible) { // phone ?>
		<td<?php echo $contact->phone->CellAttributes() ?>>
<span id="el<?php echo $contact_delete->RowCnt ?>_contact_phone" class="contact_phone">
...
<?php } ?>
<?php if ($contact->_email->Visible) { // email ?>
		<td<?php echo $contact->_email->CellAttributes() ?>>
<span id="el<?php echo $contact_delete->RowCnt ?>_contact__email" class="contact__email">
...
<?php } ?>
<?php if ($contact->send_date->Visible) { // send_date ?>
		<td<?php echo $contact->send_date->CellAttributes() ?>>
<span id="el<?php echo $contact_delete->RowCnt ?>_contact_send_date" class="contact_send_date">
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $contact_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fcontactdelete.Init();
